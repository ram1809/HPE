...
class Visualizer(object):
    def __init__(self, period, json_files):
        ...
        self.tracker.set_unit_scale(1.0)  # Convert to millimeters

        # create the background gridsVisu
        gx = gl.GLGridItem()
    ...
    def process_data(self):
        ...
        for person_id in range(output_all.shape[0]):
            ...
            # print(x3D[skeleton[0][0]-1])
            # print(y3D[skeleton[0][0]-1])
            # print(z3D[skeleton[0][0]-1])
            joint_idx = skeleton[0][0]-1  # Use the same joint you were printing
            incremental, displacement, total = self.tracker.update_position(
                    x3D[joint_idx].item(),  # Convert tensor to float
                    y3D[joint_idx].item(), 
                    z3D[joint_idx].item()
            )
            print(f"Frame {self.itert}: Movement: {incremental:.2f}mm, Displacement: {displacement:.2f}mm, Total: {total:.2f}mm")
    ...
    def update_step(self, points, lines, points_pid, lines_pid):
        ...
        if self.plotPoints is not None:
            ...
            self.plotPoints = gl.GLScatterPlotItem(pos=np.array(points), color=np.array(colors), size=5., pxMode=True)
