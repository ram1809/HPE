...
from tracker_utils import camera_matrix, get_distortion_coefficients, from_homogeneous, apply_distortion
...
from tracker_utils import camera_matrix, from_homogeneous
...
for cam_idx, cam in enumerate(parameters.cameras):
    if parameters.camera_names[cam_idx] in parameters.used_cameras:
        ...
        inverse_camera_matrices.append(torch.inverse(camera_matrix(cam)))
...
class HumanGraphFromView:
    ...
    def __init__(self, data, camera, alt):
        ...
        self.typeMap = None
        self.camera = camera
        self.camera_idx = parameters.used_cameras.index(self.camera)
    ...
    @staticmethod
    def get_cam_types():
        return parameters.used_cameras
    ...
    def get_body_rels():
        if parameters.format == 'COCO':
            ...
            return {'e_ey', 'n_ey', 'n_ne', 'ne_s', 's_el', 'el_ha', 'ne_hi', 'hi_hip', 'hip_k', 'k_a', 'a_he', 'a_fb',
                    'fb_to'}
        else:
            print(f'Unknown skeleton format in graph_generator.py {parameters.format}')
            sys.exit(-1)
    ...
    def initializeWithAlternative3(self, data):
        ...
        cam_from_root = torch.matmul(camera_i_transforms[self.camera_idx], torch.tensor(
            [0.0, 0.0, 0.0, 1.0]))  # world to camera transformation matrix, results_3d)
        self.num_joints = 0
        for j, values in data.items():
            joint = self.joints[j]
            self.features[0, all_features.index(self.camera + '_' + joint + '_i')] = (values[1] - CAMW / 2) / (CAMW / 2)
            ...
            pix_ray = torch.matmul(inverse_camera_matrices[self.camera_idx].to('cpu'),
                                   torch.tensor([values[1], values[2], 1.0]))
            undistorted_pix_ray = pix_ray  # undistort(distortion_coefficients[self.camera_idx], pix_ray)
            pix_ray_from_root = torch.matmul(camera_i_transforms[self.camera_idx], torch.cat(
                (undistorted_pix_ray, torch.tensor([0.0]))))  # perform only rotation
        ...
        for camera in sample_view:
            if camera in parameters.used_cameras:
                ...
                for idx, skeleton in enumerate(
                        json.loads(sample_view[camera][0])):  # sample_view[camera][1] is the timestamp
                    # print(camera)
                    # print(skeleton)
                    # time0 = time.time()
        ...
        for multi_person in sample_and_remove(self.limit):  # EACH TUPLE OF SAMPLE SHOULD HAVE ONE SINGLE PERSON PLUS SPURIOUS
            ...
            for sample_view in multi_person:  # FOR EACH PERSON IN THE (PLUS SPURIOUS)
                ...
                for cam_idx in sample_view:  # This loop is to get the real from the spurious heads
                    if cam_idx in parameters.used_cameras:
