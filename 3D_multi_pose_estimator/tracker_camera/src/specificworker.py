...
assert int(REALSENSE) + int(ZED) + int(OPENCV) == 1, "Only one MUST be selected, either ZED or REALSENSE."
...
if REALSENSE:
    ...
    COLOUR_WIDTH = parameters.widths[parameters.camera_names.index(hostname + '_l')]
    COLOUR_HEIGHT = parameters.heights[parameters.camera_names.index(hostname + '_l')]
elif OPENCV:
    for i in range(10):
        print(f"using camera{camera_identifier}")
    cv2grabber = cv2.VideoCapture(int(camera_identifier))
    cv2grabber.set(cv2.CAP_PROP_FRAME_WIDTH, parameters.widths[parameters.camera_names.index(hostname)])
    cv2grabber.set(cv2.CAP_PROP_FRAME_HEIGHT, parameters.heights[parameters.camera_names.index(hostname)])
...
class SpecificWorker(GenericWorker):
    ...
    def compute(self):
        if REALSENSE:
            ...
            print(type(ret), type(color_image))
            return {hostname: self.deep_compute(color_image, hostname)}
